  ...
  if (id != "" && id != undefined) {
    for (let i = 0; i < id.length; i++) {
      document.getElementById(id[i]).innerHTML =
        value[i] != undefined && value[i] != "" ? value[i] : "";
    }
  }
};
...
const tagClick = (index) => {
  document
    .getElementsByClassName("type_selected")[0]
    .setAttribute("class", "type");
  document
    .getElementsByClassName("type")
    [index].setAttribute("class", "type_selected");
  flashList(
    document.getElementsByClassName("type_selected")[0].innerHTML,
    1,
    true,
  );
  ...
  for (let i = 0; songLength > i; i++) {
    ...
    for (let n = 0; typeLength > n; n++) {
      htmType =
        htmType +
        '<span class="info_tag">' +
        response.content[i].type[n] +
        "</span>";
    }
    htmSong =
      htmSong +
      '<li class="song_item' +
      (response.content[i].notice != "" ? ' song_item_claim"' : '"') +
      " onClick=\"aimto('" +
      response.content[i].song.replace("'", "\\'") +
      "','" +
      response.content[i].uid +
      '\')" id="song_' +
      response.content[i].uid +
      '"><div class="song_info">' +
      response.content[i].song +
      " - " +
      response.content[i].singer +
      "</div>" +
      (response.content[i].notice != ""
        ? '<span class="claim">' + response.content[i].notice + "</span>"
        : "") +
...
const scrollList = () => {
  if (
    document.getElementById("songs_list").scrollTop +
      document.getElementById("songs_list").clientHeight +
      10 >=
    document.getElementById("songs_list").scrollHeight
  ) {
    if (scrollFlag == 0) {
  ...
  if (document.getElementById("search").getAttribute("mark") == "mobile") {
    if (document.getElementById("songs_list").scrollTop == 0) {
      document
        .getElementsByClassName("info_box")[0]
        .setAttribute("style", "display:inline-block;");
      document
        .getElementsByClassName("song_list")[0]
        .setAttribute("style", "top:340px;height:calc(100% - 350px);");
      document
      ...
      document
        .getElementsByClassName("song_list")[0]
        .setAttribute("style", "top:0px;height:100%;");
      ...
      if (response.code == 1) {
        if (document.getElementById("search").getAttribute("mark") == "pc") {
          ...
          alertify.alert(
            "随机抽取成功",
            "您随机到的歌曲是《" +
              response.content[0].song +
              "》-" +
              response.content[0].singer +
              ",点击OK为您自动复制到剪贴板。",
            function () {
...
const toolBox = () => {
  if (tool_box_flag == 0) {
    document
      .getElementById("tool_button")
      .setAttribute("class", "tool_button rand_animation");
    document
      .getElementsByClassName("tool_inner_item")[0]
      .setAttribute("style", "display:inline-block;");
    document
      .getElementsByClassName("tool_inner_item")[0]
      .setAttribute("class", "tool_inner_item scale_animation");
    document
      .getElementById("tool_button")
    ...
    document
      .getElementById("tool_button")
      .setAttribute("class", "tool_button rand_animation_back");
    document
      .getElementsByClassName("tool_inner_item")[0]
      .setAttribute("class", "tool_inner_item scale_animation_back");
    setTimeout(() => {
      document
        .getElementsByClassName("tool_inner_item")[0]
        .setAttribute("style", "display:none;");
...
const loadVisable = (message) => {
  let html =
    '<div class="load"><div class="loader" title="2"><svg version="1.1" id="loader-1" x="0px" y="0px" width="100px" height="100px" viewBox="0 0 50 50" style="enable-background:new 0 0 50 50;" xml:space="preserve"><path fill="#000" d="M43.935,25.145c0-10.318-8.364-18.683-18.683-18.683c-10.318,0-18.683,8.365-18.683,18.683h4.068c0-8.071,6.543-14.615,14.615-14.615c8.072,0,14.615,6.543,14.615,14.615H43.935z"><animateTransform attributeType="xml" attributeName="transform" type="rotate" from="0 25 25" to="360 25 25" dur="0.6s" repeatCount="indefinite" /></path></svg><div class="load-msg">demo</div></div></div>';
